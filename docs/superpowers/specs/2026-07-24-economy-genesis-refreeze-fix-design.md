...
**Date:** 2026-07-24
**Status:** shipped — PR #352, merged 2026-08-06 · historical draft, see §0
**Issue:** #322 (closed 2026-08-06)
...
shipped. Kept as the design record linked from #322.

- **Shipped in:** PR #352 "feat(economy): stop re-freezing genesis — record
  out-of-band burns as shrinkage, automate reconcile+audit" (`1673f6b`), merged
  2026-08-06.
- **Followed this design:** closely — all four seams landed essentially as
  written: the self-migrating `supply_changes.nft_id` column plus
  `supply_change_exists_for_nft`, the pure
  `trait_economy.burn_shrinkage_deltas` (None for a blank or unreadable
  record), the listener recorder `nft_listener._record_burn_shrinkage`,
  `supply_reconcile.reconcile_shrinkage` with
  `scripts/reconcile_supply_shrinkage.py` (dry-run default), and nightly pm2
  cron `lfg-economy-reconcile` (00:20) → `lfg-economy-audit` (00:25) with
  `audit_trait_economy.classify_drift` and an optional
  `ECONOMY_AUDIT_WEBHOOK_URL` alert.
- **Diverged:**
  - **Idempotency became a schema guarantee, not a pre-check.**
    `lfg_core/economy_store.py` adds a partial unique index
    `idx_supply_changes_burn_nft` (`kind='burn'`, `nft_id NOT NULL`) and
    `record_supply_change` uses `INSERT OR IGNORE`, so a concurrent listener
    and an `--apply` reconciler cannot both insert a shrinkage row for the same
    token. `supply_change_exists_for_nft` survives as an optimisation only.
  - **`trait_economy.body_compensation_deltas(rec, effective)` is new** and
    unplanned here: it corrects the worn-vs-recorded body mismatch that the
    `edition_bodies` pop would otherwise drift on.
  - **The listener skips a burn whose edition is still carried by a live
    duplicate token** — the chain holds multiple NFTokens per edition number, a
    case this draft did not consider.
  - **`reconcile_shrinkage` tracks covered editions in a local set updated in
    both modes**, so the dry-run preview matches exactly what `--apply` writes
    for duplicate burned tokens at a dead edition, and each row's attribute read
    is wrapped so one malformed record cannot abort the sweep.
  - **Docs landed only in CLAUDE.md.** The "freeze genesis exactly once per
    network; re-freezing is break-glass only" rule went into the trait-economy
    section of `CLAUDE.md`; `docs/runbooks/mainnet-mvp-launch.md` was not
    touched by the PR.
  - Open questions 1 and 2 shipped as proposed: a Discord webhook
    (`ECONOMY_AUDIT_WEBHOOK_URL`) rather than the bot's admin channel, and the
    nightly reconcile runs `--apply` unattended.
- **Live behaviour is now documented in:** the supply-accounting paragraph of
  CLAUDE.md's "Dress-up trait economy — Phase 2" section,
  `lfg_core/nft_listener.py::_record_burn_shrinkage`,
  `scripts/economy_nightly_reconcile.py`, and `tests/test_burn_shrinkage.py`.
