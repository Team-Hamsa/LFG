...
**Date:** 2026-07-24
**Status:** shipped — PR #358, merged 2026-08-16 · historical draft, see §0
**Issue:** #166 (closed 2026-08-16)
...
shipped. Kept as the design record linked from #166.

- **Shipped in:** PR #358 "fix(swap): precheck issuer BRIX trustline before burn,
  fall back to XRP (#166)" (`b177f76`), merged 2026-08-16.
- **Followed this design:** very closely — the closest match of this triage
  batch. Both seams landed essentially as written: `_issuer_holds_offer_trustline()`
  and the pre-burn gate in `lfg_core/swap_flow.py::run_swap_session` (same
  position, same `burn_items and pay_with == "BRIX"` condition, same AMM re-quote
  and `fee_per_nft` recompute, same clean pre-burn failure when the AMM cannot
  quote), plus the new `scripts/audit_swap_preconditions.py`. Both open questions
  resolved to the draft's recommendations: XRP graceful fallback with a loud ops
  log (Q1) and a standalone CLI audit rather than a boot check (Q2).
- **Diverged:**
  - `_issuer_holds_offer_trustline()` short-circuits to `True` when
    `config.SWAP_ISSUER_ADDRESS == config.SWAP_OFFER_ISSUER`. The draft's
    unconditional `get_trustline_balance` lookup would return `None` on the
    same-issuer (testnet) shape — an account holds no trustline to its own IOU —
    and would therefore have wrongly diverted every valid BRIX swap to XRP there.
  - The audit script has a three-value exit contract — 0 OK (including the
    same-issuer shape), 1 missing trustline with a remediation line, 2
    indeterminate lookup — rather than the draft's binary non-zero. It also
    imports `lfg_core` inside `check()` so `--network` pins `XRPL_NETWORK` before
    config freezes its endpoints and issuers.
  - The fallback log is an `OPS:`-prefixed `logging.error` naming the fix, not
    the draft's `SWAP PRECONDITION:` wording.
  - The issue's secondary DB cleanup (testnet-era `LFG` rows 3550–3554 with dead
    `minttest/` images) stayed out of scope as the draft recommended. No
    follow-up issue for it turns up in a `gh issue list --state all` search, so
    that item appears to be untracked rather than split out.
- **Live behaviour is now documented in:** the `_issuer_holds_offer_trustline`
  docstring and the inline `#166 pre-burn precondition` comment in
  `lfg_core/swap_flow.py`, and the module docstring of
  `scripts/audit_swap_preconditions.py` (which carries the exit-code contract);
  coverage in `tests/test_swap_trustline_precheck.py` and
  `tests/test_audit_swap_preconditions.py`. CLAUDE.md was not amended for this
  fix.
