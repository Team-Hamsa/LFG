...
**Date:** 2026-07-24
**Status:** shipped — PR #397, merged 2026-08-18 · historical draft, see §0
**Issue:** #220 (closed 2026-08-18)
...
shipped. Kept as the design record linked from #220.

- **Shipped in:** PR #397 "feat(mint): burn-to-mint — burn M own NFTs for M
  cap-exempt fresh mints" (`a53c4e2`), merged 2026-08-18.
- **Followed this design:** in outline only — the entitlement gate as the single
  cap-exempt construction site, burn-before-mint ordering so the collection
  never transiently exceeds the cap, and the `mint_credits` tail so a burn is
  never lost.
- **Diverged:**
  - **The burn is user-signed, not issuer-signed — the central design change.**
    This draft had the backend call `xrpl_ops.burn_nft` under issuer authority,
    reading the no-forced-burns principle as "authorization = the owner called
    the authed endpoint". Shipped, the owner signs each `NFTokenBurn` in Xaman:
    `lfg_core/xumm_ops.py::create_burn_payload` pins `Account` to the owner, and
    `burn2mint_flow` runs a **sequential** signing loop (XLS-56 Batch, which
    would collapse it to one signature, is amendment-blocked — see #219). A
    consequence: the draft's burnable-flag (`lsfBurnable`) eligibility gate is
    gone, since an owner may always burn their own token.
  - **Module and job shape.** `lfg_core/burn2mint_flow.py`, not
    `burn_mint_flow.py`. The record is a `Burn2MintSession` holding per-target
    `Burn` rows (`pending` → `awaiting_signature` → `burned` / `failed`) with
    session states `awaiting_burns` → `fulfilling` → `done`/`failed`/`cancelled`
    — there is no `VERIFYING`/`BURNING` pair; ownership verification is
    `verify_ownership()` run once in the start handler and re-checked before
    each signature.
  - **Open question 1 resolved the other way.** The mint half is not a trimmed
    copy of the fulfillment loop. `burn2mint_flow.build_mint_job` constructs a
    real `bulk_mint_flow.BulkMintJob` in state `PAID` carrying the burn
    entitlement, under a deterministic id (`bulk_job_id_for`, `b2m<sid>`), so
    bulk mint's per-unit persistence, startup resume, re-offer pass and credits
    tail are reused verbatim rather than reimplemented.
  - **`build_burn_entitlement` signature.** Shipped it takes only
    `burn_nft_ids` (quantity is derived, duplicates rejected); the draft passed
    `(quantity, burn_nft_ids)` and asserted equality.
  - **Endpoints and client.** `/api/mint/burn2mint` with `start` / `{id}` status
    / `active` / `cancel` (`lfg_service/app.py::handle_burn2mint_*`), not
    `/api/mint/burn`; there is no dedicated per-unit accept endpoint (the reused
    `BulkMintJob` handles delivery), and **no client UI shipped** — `burn2mint`
    appears nowhere under `webapp/client/`.
  - **Open question 2** resolved as reuse: the per-request cap is
    `config.BULK_MINT_MAX`, not a distinct `BURN_MINT_MAX`.
  - **Open question 4** resolved as yes: it ships dark behind
    `BURN_TO_MINT_ENABLED` (`lfg_core/config.py`, default `"0"`; new sessions
    403 while off, resume of already-burned sessions ignores the flag).
  - **Crash-safety machinery the draft did not anticipate**, added over five
    review rounds: persist-before-expose (a signable payload cannot exist
    without a durable record); a dedicated **orphan-payload journal**
    (`orphan-payload-<uuid>.json`) for the case where the session record cannot
    be written and the XUMM cancel is unconfirmed; `recover_orphan_payloads()`
    wired into `resume_all` to resolve each journal automatically (validated
    orphan burn → durable `mint_credits` row; anything indeterminate is kept,
    fail-closed); adopt-before-create reconversion so a re-convert cannot
    clobber a resumed job's minted units; and `mint_credits.add_credit(source_key=...)`
    with a self-migrating `mint_credit_sources` dedup table so credit-then-retire
    converges instead of double-crediting.
  - Open questions 3 (listed/blank characters) and 5 (rarity feedback) are not
    visibly answered in the shipped code; eligibility is ownership plus
    membership only.
- **Live behaviour is now documented in:** the module docstring of
  `lfg_core/burn2mint_flow.py`, the `BURN_TO_MINT_ENABLED` comment block in
  `lfg_core/config.py`, the CLAUDE.md env table entry for that flag, and
