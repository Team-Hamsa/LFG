...
**Date:** 2026-07-24
**Status:** shipped — PR #376, merged 2026-08-17 · historical draft, see §0
**Issue:** #221 (closed 2026-08-17)
...
shipped. Kept as the design record linked from #221.

- **Shipped in:** PR #376 "Session resume for swap/market/economy/shop flows
  after Activity relaunch" (`b283a77`), merged 2026-08-17.
- **Followed this design:** closely. The consolidated `GET /api/sessions/active`
  endpoint (`lfg_service/app.py::handle_sessions_active`), the additive `kind`
  keys on every market `to_dict()` and on
  `webapp/economy_api.py::economy_session_dict`, the Node-testable priority
  picker (`webapp/client/resume_pure.js::pickActiveFlow`, same `FLOW_ORDER`
  mint > bulk > swap > market > economy > shop), and the single
  `webapp/client/app.js::resumeAnyFlow()` boot dispatcher replacing the
  `resumeBulkMint()/resumeMint()` chain all landed as drafted. Open question 3
  (economy resume UX) resolved to the draft's assumption — a compact
  reconnect→result overlay (`attachEconomyResume`), not the full Dressing Room
  panel. Open question 4 resolved to keep both `/api/mint/active` and
  `/api/mint/bulk/active` registered.
- **Diverged:**
  - Auth: the draft's sketch used `@require_auth`; the shipped handler is
    `@require_wallet`, because the shop lookup needs the resolved wallet.
  - Shop is **not** picked via `_active_session`. `ShopBuySession` carries no
    platform-user identity, so the handler scans `shop_sessions` for
    `s.buyer == request["wallet"]` with a matching `platform`, mirroring
    `handle_shop_buy_start`'s own guard.
  - The shipped handler prunes all six stores (mint and bulk included); the
    draft's sketch pruned only swap/market/economy/shop.
  - Swap re-attach does not call `openSwapper()`. `app.js::attachSwapResume`
    shows `swap-result-panel` with a "Reconnecting…" title and polls; the swap
    picker is never opened over a live session.
  - Market re-attach renders the resumed session dict immediately
    (`showFlow(render(session))` in `attachMarketResume`) before starting the
    poll, instead of waiting for the first 3 s tick.
  - `BuySession.to_dict()` additionally gained `listing_kind` — `marketBuyRender`
    is a per-listing-kind factory, so `MARKET_RESUME_RENDER.buy` needs it to
    rebuild the right renderer for a resumed buy.
  - The client shop terminal set is `{done, failed}`, matching
    `lfg_core/shop_flow.py::TERMINAL_STATES`. The draft's `expired`/`settled`
    were `shop_orders` row statuses, not session states.
  - Substantial concurrency hardening the draft did not anticipate (added under
    PR review): `invalidateFlowPolls()` plus a `flowRenderGen` shared-panel
    ownership counter, new `marketFlowGen`/`shopFlowGen` generation guards on the
    market and shop pollers (a tick already awaiting its fetch survives
    `clearTimeout` and would otherwise repaint over a newly attached flow),
    `resume_pure.js::hasOtherActiveFlow` plus a one-shot `resumeRecheckArmed`
    re-check in `showMintHome()` so a second live flow surfaces once the first
    finishes, and `exitSwapAfterCancel()` to dedupe the two paths that can
    observe the same swap cancellation.
- **Live behaviour is now documented in:** the `handle_sessions_active`
  docstring (`lfg_service/app.py`), the `webapp/client/resume_pure.js` module
  header, and the `resumeAnyFlow()` comment block in `webapp/client/app.js`;
  coverage in `tests/test_session_resume.py` and `tests/test_resume_pure_js.py`.
  CLAUDE.md was not amended for this feature.
