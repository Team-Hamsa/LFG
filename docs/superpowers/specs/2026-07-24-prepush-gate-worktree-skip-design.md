...
**Date:** 2026-07-24
**Status:** shipped — PR #373, merged 2026-08-16 · historical draft, see §0
**Issue:** #315 (closed 2026-08-16)

## 0. What actually shipped (reviewed 2026-08-19)

Drafted 2026-07-24 during a backlog triage sweep and never merged before the
work shipped. Kept as the design record linked from #315.

- **Shipped in:** PR #373 "fix(gate): resolve pre-push venv via
  scripts/venv-python so worktrees run the real gate" (`b2d7e12`), merged
  2026-08-16.
- **Followed this design:** almost verbatim. `scripts/venv-python` resolves
  `<git rev-parse --git-common-dir>/../.venv/bin/python`, `exec`s it, and
  hard-fails non-zero naming the exact path plus "run ./setup.sh from
  <main checkout>" when it is missing; the hook `entry:` lines in
  `.pre-commit-config.yaml` call the shim; `setup.sh` gained the clarifying
  comment only. All three open questions resolved as recommended — shared shim
  over an inline `bash -c`, always the common-dir venv (never a worktree-local
  one), and no worktree guard added to `setup.sh`.
- **Diverged:**
  - Four hook entries were rewired, not the three listed here:
    `audit-layer-dimensions` landed after 2026-07-24 with the same
    `.venv/bin/python` entry and this doc predates it. A fifth,
    `check-repo-layout`, arrived from PR #369 twenty minutes before #373 merged
    and was converted to the shim shortly after by PR #374 (`37cf7b8`), so
    `.pre-commit-config.yaml` now carries five `scripts/venv-python` entries.
  - Shim internals differ cosmetically: `main_root="$(cd "$(dirname
    "$common_dir")" && pwd)"` instead of the draft's `cd "$common_dir/.."`
    (equivalent), and the failure message is three plain `echo`s rather than a
    heredoc, without the draft's closing "Do not bypass the gate with
    --no-verify" line. Separately, the PR's last commit removed the pre-existing
    `# Bypass in genuine emergencies with: git push --no-verify` comment from
    `setup.sh`.
  - `tests/test_venv_python_shim.py` has six tests, not the draft's three: it
    adds an executable-bit check and argument forwarding, and every subprocess
    scrubs `GIT_*` from the environment — the installed pre-push hook exports
    `GIT_DIR`, which otherwise leaks into the temp repos the worktree tests
    build.
  - PR #373 also deleted unrelated tracked artifacts (`reports/body_affinity*`,
    two 2026-06-23 economy report snapshots, `.superpowers/sdd/progress.md`).
    Housekeeping, not part of this design.
- **Live behaviour is now documented in:** the header comment of
  `scripts/venv-python` and the `#315` note above the `pre-commit install` step
  in `setup.sh`. CLAUDE.md's "Pre-push gate (BLOCKING)" section still describes
  mypy as running "from the project `.venv`" and does not mention the shim.
