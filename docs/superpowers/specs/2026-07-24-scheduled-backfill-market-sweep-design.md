...
**Date:** 2026-07-24
**Status:** shipped — PR #375, merged 2026-08-16 · historical draft, see §0
**Issue:** #288 (closed 2026-08-16)

## 0. What actually shipped (reviewed 2026-08-19)

Drafted 2026-07-24 during a backlog triage sweep and never merged before the
work shipped. Kept as the design record linked from #288.

- **Shipped in:** PR #375 "Scheduled nightly backfill_market sweep (#288)"
  (`68b6308`), merged 2026-08-16.
- **Followed this design:** both seams as specified.
  `scripts/backfill_market.py` gained `logging.basicConfig` in `_amain`, the
  human summary emitted through `logging`, and `_log_summary`'s greppable
  `backfill_market drift: net=… closed_stale=… bids_closed_stale=…
  fetch_failures=… bid_fetch_failures=… live_listings=…` line — WARNING when any
  of the four counters is non-zero, INFO otherwise — plus a default-off
  `--report` flag whose `_append_drift_report` appends one JSON line per run to
  the gitignored `reports/backfill_market_drift.log`. Both ecosystem files
  gained a `cron_restart: "30 3 * * *"`, `autorestart: false` entry passing
  `--network <net> --report`. Open questions 1, 2, 3 and 5 were resolved as
  proposed: nightly, 03:30, a per-connection `busy_timeout` rather than a
  repo-wide WAL switch, and both stacks wired for parity.
- **Diverged:**
  - The pm2 processes are named `lfg-market-sweep` / `stg-market-sweep`, not
    this draft's `lfg-market-backfill` / `stg-market-backfill`.
  - The `busy_timeout` is not a `conn.execute` in `_amain`. PR #375 added an
    optional `busy_timeout_ms` parameter to `lfg_core/nft_index.py::init_db` and
    the backfill threads `BUSY_TIMEOUT_MS = 30000` through it, so the pragma is
    applied **before** the schema DDL — otherwise initialization itself could
    still die on sqlite's 5 s default.
  - The draft did not anticipate the timezone caveat that CLAUDE.md now carries:
    pm2's `cron_restart` has no timezone option and fires in host-local time, so
    the 03:30 slot is only UTC because the deploy box runs `Etc/UTC`.
  - Open question 4 (alerting depth) shipped at the MVP: WARNING log line plus
    the `reports/` record, no webhook.
- **Live behaviour is now documented in:** CLAUDE.md's "Running (two pm2 stacks,
  branch-driven — #223)" section — the pm2 table rows and the `lfg-market-sweep`
  paragraph covering the 03:30 slot, the host-local cron caveat, the
  "parks stopped between runs" note, the greppable drift line and drift log, and
  the per-stack `pm2 start ecosystem.<stack>.config.js --only <name> && pm2
  save` activation step (adding an app to an ecosystem file does not start it
  on a running stack).
