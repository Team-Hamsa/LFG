...
**Date:** 2026-07-24
**Status:** shipped — PR #360, merged 2026-08-16 · historical draft, see §0
**Issue:** #152 (closed 2026-08-16)
...
shipped. Kept as the design record linked from #152.

- **Shipped in:** PR #360 "feat(xumm): cancel the open XUMM payload on
  mint/bulk/swap session cancel" (`cbe55af`), merged 2026-08-16.
- **Followed this design:** closely. `lfg_service/app.py::_spawn_payload_cancel`
  is the fire-and-forget helper with the retained-task set and the `None`-uuid
  no-op, wired into `handle_mint_cancel`, `handle_bulk_mint_cancel` and
  `handle_swap_cancel` right after the session/job cancel succeeds;
  `SwapSession` gained the `payment_uuid` capture this draft identified as the
  one blocker for swap. Open question 1 answered "fire-and-forget", open question
  2 answered "mint + bulk + swap in one PR", open question 3 answered "log-only".
- **Diverged:**
  - The helper wraps the call in its own `async def _cancel()` with a defensive
    `try/except` logging at WARNING. This draft handed the coroutine straight to
    `create_task`, which would have surfaced any unexpected raise as an
    unretrieved task exception.
  - **Superseded payloads were added beyond this draft's scope.** `MintSession`
    and `SwapSession` now carry `stale_payment_uuids` — a payload that a QR
    regeneration replaced otherwise stays signable at XUMM until its own expire —
    and `_spawn_stale_payload_cancels(session)` drains that list on both the
    cancel and the regenerate handlers for mint and swap. Bulk cancel still
    cancels only the single `job.payment_uuid`.
  - Both flows also close the cancel-racing-regeneration window: a replacement
    payload that lands after the session has gone terminal is parked as stale
    rather than left live (`mint_flow.MintSession.regenerate_payment`,
    `swap_flow.SwapSession.regenerate_payment`).
  - The follow-up case raised on the issue after this draft was written —
    surfaces that abandon a mint without ever cancelling the session, which needs
    a `cancel_mint` method on `surfaces/_client/client.py` — was not addressed;
    the surface SDK still has no mint-cancel method, so the abandon path never
    reaches this wiring.
  - Market list/buy/cancel and the trait-sell wizard stayed out of scope, as
    proposed here.
- **Live behaviour is now documented in:** the docstrings of
  `_spawn_payload_cancel` and `_spawn_stale_payload_cancels` in
  `lfg_service/app.py`, and the `#152` comments in `lfg_core/mint_flow.py` and
