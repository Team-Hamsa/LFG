...
**Date:** 2026-07-24
**Status:** shipped — PR #393, merged 2026-08-18 · historical draft, see §0
**Issue:** #273 (closed 2026-08-18)
...
shipped. Kept as the design record linked from #273.

- **Shipped in:** PR #393 "feat(mint): share-link mint attribution + conversion
  metrics (#273)" (`d944bd7`), merged 2026-08-18.
- **Followed this design:** in outline — the four-seam shape held (client sends
  the stashed ref → service validates → threaded through the mint path →
  conversion readout), and the durable store is the self-migrating nullable
  `LFG.referrer` column added in `lfg_core/db_helpers.py::record_nft_mint`
  exactly as drafted. Self-referral and checksum validation are enforced
  server-side, and a bad ref never fails a mint.
- **Diverged:**
  - The wire field is `ref`, not `referrer`
    (`webapp/client/app.js::startMint` / `startBulkMint`;
    `lfg_service/app.py::_mint_referrer`).
  - No `lfg_core/referral.py` exists. Validation lives in
    `lfg_service/app.py::_mint_referrer`; the aggregate lives in
    `lfg_core/share_clicks.py::conversion_rows`.
  - **The metrics surface is the opposite of what the draft chose.** The draft
    argued for an ops-CLI-only readout (`scripts/share_metrics.py`) and
    explicitly refused a public board. What shipped is a public, unauthenticated
    `GET /api/share/conversions` (`lfg_service/app.py::handle_share_conversions`,
    60 s per-network cache); no `scripts/share_metrics.py` exists in the tree.
    Rows are `{wallet, clicks, mints}` — the draft's computed
    `conversion_rate` field was dropped. PR #393's body argues the abuse surface
    is bounded because attribution stays rewards-free and bot clicks are
    excluded.
  - Bulk mint was wired end-to-end **and durably**, which the draft deferred:
    `BulkMintJob.referrer` round-trips through `to_dict()`/`from_dict()` (so it
    survives crash/resume of the on-disk job record) and `_fulfill_unit` stamps
    every unit's `LFG` row. The draft's out-of-scope note expected bulk to pass
    `None` or a non-durable ref.
  - Open question 3 (ref freshness) was answered yes, and differently from the
    proposal: the stash became `{ref, ts}` JSON with a 24 h `REF_TTL_MS` window
    (legacy plain-string stashes are treated as expired), and `consumeRef()`
    clears it only once a status poll observes a minted outcome (`offer_ready`,
    or bulk `minted > 0`) — not on mint start as the draft proposed.
  - The thread through `lfg_core/mint_flow.py` is wider than drafted because the
    #336 refactor had landed in between: `referrer` also passes through
    `_finalize_minted_unit` and `_resume_prepared_mint_one_unit` (the sponsored
    resume path), not only `mint_one_unit`.
- **Live behaviour is now documented in:** the PR #393 description, the
  `_mint_referrer` / `handle_share_conversions` docstrings in
  `lfg_service/app.py`, `lfg_core/share_clicks.py::conversion_rows`, and the
  `stashedRef()` / `consumeRef()` comment block in `webapp/client/app.js`;
  coverage in `tests/test_mint_ref_attribution.py`. CLAUDE.md was not amended
  for this feature.
