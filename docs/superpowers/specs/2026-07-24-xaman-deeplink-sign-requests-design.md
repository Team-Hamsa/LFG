...
**Date:** 2026-07-24
**Status:** shipped — PR #380, merged 2026-08-17 · historical draft, see §0
**Issue:** #142 (closed 2026-08-17)
...
shipped. Kept as the design record linked from #142.

- **Shipped in:** PR #380 "Mobile-primary Xaman deep links on every sign request"
  (`fc518c8`), merged 2026-08-17.
- **Followed this design:** Seam A landed essentially as written —
  `isCoarsePointer()` over `matchMedia('(pointer: coarse)')` with no UA sniffing,
  `applySignDelivery({qrEl, linkBtn, toggleBtn, link, qrData, push})` as the single
  DOM choke point, `maybeAutoOpen(link)` deduped per payload link, the
  `push === 'sent'` → collapse-the-QR-and-do-not-auto-open rule, desktop left
  QR-primary, and the canonical `next.always` deep link with no `xumm://` scheme and
  no LFG-hosted redirect hop. Seam C shipped as the explicit documented deferral it
  was specced to be.
- **Diverged:**
  - **The truth table was extracted into a pure module.** Not in the draft:
    `webapp/client/signdelivery_pure.js` exports `signDelivery({push, coarse,
    hasLink, hasQr}) -> {linkPrimary, qrCollapsed, autoOpen}`, plus
    `shouldAutoOpen` and `autoOpenOutcome`, executed under Node by
    `tests/test_signdelivery_pure_js.py` (the `*_pure.js` pattern already used by
    `mint_pure.js`). It encodes one rule the draft did not state: never collapse
    the QR when it is the only affordance left (no link).
  - **Blocked-launch retry.** `autoOpenOutcome(seen, link, launched)` un-marks a
    link when the launch was *detectably* blocked (`window.open` returned null), so
    a later render may retry. The draft only said auto-open was "best-effort".
  - **Seam B was not done.** `surfaces/discord_bot/trustline.py` still hand-rolls
    its XUMM POST and reads `response_data["refs"]["qr_png"]` and
    `["next"]["always"]` itself; it was never folded onto `_create_xumm_payload`,
    so it still has no push delivery, no `push` state, and keeps its own
    `expire: 5`. PR #380's only Discord change is relabelling the embed link to
    "Open in Xaman" in `surfaces/discord_bot/views.py`. The draft's premise for
    Seam B was also partly wrong: that hand-rolled `TrustSet` txjson already
    carried `SourceTag` and provenance `Memos` inline (added for #54 by PR #144),
    so the invariant was never actually broken there — only push and the shared
    builder's other defaults were missing, and they still are.
  - **Telegram was in scope after all.** The draft covered only the Activity
    client, the Discord trustline and the CLI deferral. PR #380 also added tappable
    `Open in Xaman: <link>` lines to the Telegram swap-fee and swap-claim captions
    (`surfaces/telegram_bot/swap_render.py`, `swap_view.py`) — both had been
    QR-only dead ends on mobile — and relabelled the mint captions in
    `surfaces/telegram_bot/render.py`.
  - **Open question 1 resolved toward "always on".** The draft recommended
    auto-open ON for payment/accept screens and OFF for passively-reached ones.
    `applySignDelivery` does take an `autoOpen = true` opt-out parameter, but
    verified against the tree on 2026-08-19 no call site passes `autoOpen: false` —
    every wired sign panel auto-opens once on a coarse pointer.
  - **Open question 2 resolved as coarse-pointer-only.** There is no Telegram Mini
    App / mobile-web override; pointer type is the sole signal.
  - **Open question 5 (scope trim) resolved by omission:** Seam A alone closed
    #142; Seam B was simply dropped rather than split to its own issue.
  - **Cache-buster numbers moved on.** The draft cited `app.js?v=32`; PR #380
    bumped `v=51` → `v=52` and renamed the stylesheet to `style.v24.css`. The tree
    sits at `app.js?v=59` today after later, unrelated PRs.
- **Live behaviour is now documented in:** the header comment of
  `webapp/client/signdelivery_pure.js` and the `#142` comments at each
  `applySignDelivery` call site in `webapp/client/app.js`. CLAUDE.md's XUMM push
  section (#135/#212) still describes delivery only; it does not mention the
  mobile-primary presentation.
