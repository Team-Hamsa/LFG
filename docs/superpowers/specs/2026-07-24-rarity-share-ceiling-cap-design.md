...
**Date:** 2026-07-24
**Status:** shipped — PR #394, merged 2026-08-18 · historical draft, see §0
**Issue:** #198 (closed 2026-08-18)

## 0. What actually shipped (reviewed 2026-08-19)

Drafted 2026-07-24 during a backlog triage sweep and never merged before the
work shipped. Kept as the design record linked from #198.

- **Shipped in:** PR #394 "feat(rarity): share-ceiling cap on trait weights"
  (`b3fbe9f`), merged 2026-08-18.
- **Followed this design:** the clamp is as specified.
  `lfg_core/rarity.py::effective_weight` gained `candidate_count` and
  `cap_multiple`, computes `ceiling = max(cap_multiple / candidate_count,
  floor_weight)` then `base = min(max(share, floor_weight), ceiling)` — after
  the Laplace-smoothed share, before `boost_multiplier` — and `weighted_pick`
  passes `candidate_count=len(rows)`, the enabled candidates in the actual
  pick.
  `lfg_core/config.py` gained `RARITY_CAP_MULTIPLE`, default 0 = off. Open
  questions 1 and 2 were ratified as recommended (fair-share basis, plateau,
  0-gate; no hysteresis needed).
- **Diverged:**
  - `cap_multiple` defaults to `None`, not `0.0`, and `effective_weight` reads
    `config.RARITY_CAP_MULTIPLE` itself when the argument is `None`. So no call
    site threads the knob as the draft showed — `weighted_pick`, `get_odds` and
    `scripts/trait_dashboard.py` pass only `candidate_count`, and the multiple
    is read live per pick (no restart, matching how the rest of the rarity
    table is read).
  - The knob is defined against a named `RARITY_CAP_MULTIPLE_DEFAULT = 0.0`
    constant so a test can lock the shipped default — the #323
    test-env-isolation convention, which this draft predates.
  - Open question 4 was answered the opposite way to the draft's proposal. The
    admin views do **not** approximate `candidate_count` with the enabled-row
    count: PR #394 added `lfg_core/rarity.py::available_values` (backed by a new
    sync `LocalLayerStore.list_values_sync` in `lfg_core/layer_store.py`) so
    `get_odds` and the dashboard count enabled ∩ layer-store-available. The
    enabled-only count survives only as the fallback when the candidate set
    can't be resolved (non-local `LAYER_SOURCE`, legacy `'*'` body, missing
    dir). That parity is still not exact:
    `lfg_core/traits.py::select_random_attributes` filters `available` through
    `trait_config` affinity/conflict rules before calling `weighted_pick`, and
    `available_values` does not reproduce that filtering. Greptile raised this
    on PR #394 and it merged with the gap open.
  - Open question 3 (surface the ceiling itself) shipped as neither variant:
    `get_odds` keeps its 5-tuple `(trait, live_count, share%, weight, status)`
    and no `ceiling` / `capped` field or badge was added to
    `scripts/trait_dashboard.py`. An operator sees only that the displayed
    weight stops rising.
  - The cap ships dark: `RARITY_CAP_MULTIPLE` is 0 on every stack until an
    operator sets it (3.0 remains the recommended opt-in value), and open
    question 5 — whether to re-enable the manually parked traits and let the cap
    govern them — is still an open ops decision.
- **Live behaviour is now documented in:** the module docstring and the
  `effective_weight` docstring in `lfg_core/rarity.py`, plus the comment above
  `RARITY_CAP_MULTIPLE` in `lfg_core/config.py`. The knob is not listed in
  CLAUDE.md's environment-variable block.
