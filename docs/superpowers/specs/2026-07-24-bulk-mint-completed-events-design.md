...
**Date:** 2026-07-24
**Status:** shipped — PR #392, merged 2026-08-18 · historical draft, see §0
**Issue:** #253 (closed 2026-08-18)
...
shipped. Kept as the design record linked from #253.

- **Shipped in:** PR #392 "feat(events): publish per-unit mint.completed for
  bulk-mint units" (`b107309`), merged 2026-08-18.
- **Followed this design:** closely. Option 1 (per-unit events) as specified —
  emit at `OFFERED`, a durable per-unit `published` flag set only after a
  successful publish, `Unit` carrying `traits`/`body_type` captured in
  `_fulfill_unit`, the publisher owned by the service and injected down into
  `lfg_core`, and zero consumer edits.
- **Diverged:**
  - **Names and injection style.** The sweep is
    `bulk_mint_flow.publish_unit_events(job)`, not `emit_completed`, and the
    hook is a **module-level** `bulk_mint_flow.unit_event_publisher` assigned
    once at import (`lfg_service/app.py`: `bulk_mint_flow.unit_event_publisher =
    _publish_bulk_unit`) rather than a per-job `job.on_unit_complete` set at
    each `run_bulk_mint_job` launch site.
  - **Publish failure defers the job instead of dropping the event.** The draft
    let a failed publish log-and-continue, with the job free to reach `DONE`.
    Shipped, `bulk_mint_flow.events_pending(job)` gates the terminal
    transition: a job holding an OFFERED-but-unpublished unit stays
    `FULFILLING` so the startup sweep retries, because `DONE` records are
    pruned and never resumed — terminalizing would suppress the event forever.
  - **A durability precondition the draft lacked.** `publish_unit_events`
    refuses to announce an `OFFERED` state that is not on disk: on
    `job.persist_failed` it retries `persist(job)` once and otherwise defers the
    whole sweep, so a crash+resume that reloads pre-OFFERED units cannot
    double-announce.
  - **Payload shape.** The draft's small dict plus a `"bulk": True` marker
    became full key parity with `MintSession.to_dict()` in
    `lfg_service/app.py::_bulk_unit_event_data` — every session key present
    (session-only fields at their defaults, payment facts from the job), `id` =
    `f"{job.id}:{unit.index}"`, and `bulk_job_id` + `unit_index` as the bulk
    markers. Parity is test-enforced, so a new `MintSession` field fails tests
    until it is accounted for.
  - **`video_url` is not persisted on `Unit`.** The draft proposed capturing
    `res.video_url` alongside `traits`/`body_type`; the shipped `Unit` carries
    only `traits` and `body_type`, and `_bulk_unit_event_data` emits
    `"video_url": None`. A bulk-minted animated edition therefore announces
    without its MP4.
  - Open questions 1, 2 and 3 shipped as the draft proposed: no `mint.failed`
    for bulk units, no one-shot "pre-existing OFFERED = already published"
    migration, and no per-bulk-job X budget cap.
- **Live behaviour is now documented in:** the docstrings of
  `lfg_core/bulk_mint_flow.py::publish_unit_events` / `events_pending` and
  `lfg_service/app.py::_publish_bulk_unit` / `_bulk_unit_event_data`; tests in
  `tests/test_bulk_mint_events.py`.
