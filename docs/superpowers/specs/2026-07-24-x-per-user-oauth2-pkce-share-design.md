...
**Date:** 2026-07-24
**Status:** shipped — PR #398, merged 2026-08-18 · historical draft, see §0
**Issue:** #252 (closed 2026-08-18)
...
shipped. Kept as the design record linked from #252.

- **Shipped in:** PR #398 "feat(x): per-user OAuth2 PKCE — share from my account
  (#252)" (`e9ef5b1`), merged 2026-08-18.
- **Followed this design:** closely on the security core — OAuth2 Authorization
  Code + PKCE with S256, Fernet-encrypted tokens at rest in an `x_accounts` table
  keyed by wallet, rotate-and-persist-before-use refresh, a public callback whose
  one-shot `state` is the only auth, unconditional local delete on disconnect,
  feature-off-when-unset, no XRPL transaction anywhere (so no SourceTag/memo
  surface). `cryptography` was pinned in `requirements.txt` as specced.
- **Diverged:**
  - **One module, not two.** The draft split an encrypted store
    (`lfg_core/x_token_store.py`) from the OAuth/posting client
    (`lfg_core/x_oauth.py`). Everything landed in a single
    `lfg_service/x_oauth.py`: `ensure_x_accounts_table`, `upsert_account`,
    `rotate_tokens`, `get_account`, `delete_account` alongside
    `begin_authorization`, `claim_state`, `exchange_code`, `refresh_access_token`,
    `revoke_token`, `fetch_me`, `post_tweet` and `get_usable_access_token`.
  - **Env var names differ, and there is no master flag.** Shipped names are
    `X_OAUTH_CLIENT_ID`, `X_OAUTH_CLIENT_SECRET`, `X_OAUTH_CALLBACK_URL`,
    `X_TOKEN_ENC_KEY` (the draft proposed `X_OAUTH2_CLIENT_ID/SECRET` and
    `X_OAUTH2_REDIRECT_URI`). `config.X_USER_SHARE_ENABLED` is not an env flag at
    all — it is derived, `all((X_OAUTH_CLIENT_ID, X_OAUTH_CALLBACK_URL,
    X_TOKEN_ENC_KEY))` — so the client secret is optional (public PKCE clients omit
    it) and there is no knob to force the feature off while the creds are present.
  - **Feature-off is 404, not 403.** `app.py::require_x_user_share` makes all five
    `/api/x/*` handlers 404, indistinguishable from a missing route.
  - **No media upload, and the tweet carries no link.** The draft's share step
    downloaded the NFT image, called `upload_media` and posted `text + public card
    URL`. What shipped is `post_tweet(access_token, text)` only — there is no media
    helper in the module — and `app.py::_x_share_text` builds the text server-side
    and deliberately **link-free**, per the 2026-07-17 brand cost directive (X
    bills ~$0.20/post containing a URL vs ~$0.015 without). That also disposes of
    draft open question 5 (`?ref=` attribution on a server-posted card URL): there
    is no URL to attribute.
  - **Spend control is a cooldown plus dedup, not a budget ledger.** The draft's
    O1 recommendation (option b) was an `x_share_log` table with
    `X_USER_SHARE_MONTHLY_BUDGET` and per-user caps. Shipped instead: two
    in-memory guards in `app.py` — `_x_share_last` (per-wallet cooldown,
    `X_USER_SHARE_COOLDOWN_SECONDS`, default 60) and `_x_share_posted`
    (per-`(wallet, kind, nft_number)` dedup that returns the cached tweet id rather
    than paying for a second post, `X_USER_SHARE_DEDUP_SECONDS`, default 86400).
    No durable log, no monthly cap, and a restart resets both — documented in the
    code as risking at most one extra paid post per wallet. Open question 2 (paid
    tier vs pay-per-use) is therefore still unanswered in code.
  - **Status endpoint renamed.** `GET /api/x/account`, not the draft's `GET
    /api/x/status`. The full route set is `GET`/`DELETE /api/x/connect`, `GET
    /api/x/callback`, `GET /api/x/account`, `POST /api/x/share`.
  - **State is an opaque in-memory handle, not an HMAC-signed token.** The draft
    reused `make_session_token`'s HMAC construction to carry the wallet inside
    `state`, with a separate nonce→verifier map. Shipped: `begin_authorization`
    mints a `secrets.token_urlsafe(32)` state and stores
    `PendingAuth(wallet, code_verifier, created_at)` in the process-local
    `pending_auths` dict; `claim_state` pops it (one-shot, unreplayable) and
    enforces `STATE_TTL_SECONDS = 600`. A restart mid-consent simply makes the user
    reconnect — the posture the draft accepted for the verifier map, now applied to
    the wallet binding too.
  - **Table shape.** Ciphertext is stored as TEXT, not BLOB; `refresh_token_enc` is
    nullable; `expires_at` is REAL; there is no `updated_at` column.
  - **Share request body.** The draft took `{"nft_number": N}`. Shipped takes
    `{"kind": "mint"|"swap", "nft_number": int|null}`, 400s an unknown kind, and
    explicitly rejects a bool `nft_number` (which would otherwise tweet "#True" at
    real cost).
- **Live behaviour is now documented in:** the `X_OAUTH_*` / `X_TOKEN_ENC_KEY`
  entries in CLAUDE.md's env-var block (each tagged `#252`), and the module
  docstring and comments in `lfg_service/x_oauth.py`. The feature ships dark: it
  stays off until an operator registers a public HTTPS callback on the X developer
  portal and sets a Fernet key.
