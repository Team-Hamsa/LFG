...
**Date:** 2026-07-24
**Status:** shipped — PR #396, merged 2026-08-18 · historical draft, see §0
**Issue:** #207 (closed 2026-08-18)
...
shipped. Kept as the design record linked from #207.

- **Shipped in:** PR #396 "feat(identity): first-class user profiles above
  per-platform identities" (`811b7cd`), merged 2026-08-18.
- **Followed this design:** in outline only. The `profiles` table, membership as
  the reserved `identities.account_id` FK rather than a `profile_wallets` join
  table (decision O4), the self-migrating `ensure_identities_table()` pattern and
  the `idx_identities_account` index all landed as specced, in
  `lfg_service/identity.py`.
- **Diverged:**
  - **Built on #206, not ahead of it.** The draft's O1 recommendation was option
    (a): ship a degenerate one-profile-per-wallet layer now, let #206 widen
    membership later. The opposite happened — #206's `wallet_links` history and
    bucket resolver landed first (PR #395, merged 2026-08-18, `f715b95`) and #207
    was written on top of them. Membership is bucket-wide from the first write:
    `identity.ensure_profile_for` resolves the identity's bucket
    (`_bucket_on_conn`, on its own `BEGIN IMMEDIATE` connection) and attaches to a
    bucket-mate's existing profile, unifying several if the bucket spans more than
    one.
  - **Keyed on the identity, not the wallet.** The draft's helpers were
    `ensure_profile_for_wallet(wallet)`, `profile_for_wallet(wallet)`,
    `profile_for_identity(...)`, `link_identity_to_profile(...)` and
    `update_profile(...)`. What shipped is `ensure_profile_for(platform,
    platform_user_id)`, `profile_for(platform, platform_user_id)` and
    `merge_profiles(a, b)`. There is no `update_profile`, no
    `link_identity_to_profile`, and no wallet-keyed entry point at all; wallets
    come back as a derived field of `profile_for()` via the #206 bucket.
  - **Merge semantics are richer than the draft's.** The draft deferred merge
    policy entirely to #206. The shipped module adds an append-only `merged_into`
    column (a losing row is never deleted, so historical `account_id` values stay
    resolvable through `_resolve_merged`, and re-merging is a no-op), a
    deterministic oldest-wins rule, and a conflict report for differing
    `display_name` / `avatar_url` / `preferences` instead of silently merging them.
  - **Fail-closed, not the module's never-raises house style.** The draft specced
    helpers in `identity.py`'s "try/except-log-return-falsy" style. The shipped
    ones raise `ProfileError` and propagate `BucketLookupError`, so an
    infrastructure error can never silently create or attach a wrong profile; an
    unknown identity raises rather than returning falsy. The module comment calls
    this out as a deliberate exception to its own style.
  - **Schema differs.** Shipped columns are `id`, `display_name`, `avatar_url`,
    `preferences TEXT NOT NULL DEFAULT '{}'`, `merged_into`, `created_at`. The
    draft's `updated_at` is absent; `merged_into` is not in the draft.
  - **None of the read/write surface shipped.** There is no `GET`/`PATCH
    /api/profile` route, no SDK `profile`/`update_profile` client method, and
    `app.py::_lb_display_name` and `enrich_minter_identity` still resolve names via
    `identity_store.handle_for_wallet` — the profile-first display-name upgrade the
    draft called "the concrete per-human payoff" was not done. Nor is
    `ensure_profile_for` wired into any `link(...)` site, so decision O2
    (eager-vs-lazy auto-create) is moot: verified against the tree on 2026-08-19,
    the only callers of any profile helper anywhere are in `tests/test_profiles.py`.
    PR #396 states this deliberately ("No live behavior change"); profiles are a
    foundation still waiting on a consumer.
- **Live behaviour is now documented in:** the `#207` block comment above
  `ProfileError` in `lfg_service/identity.py` (and the `#206` bucket block just
  above it). CLAUDE.md does not mention profiles — nothing user-visible reads them
  yet.
