...
**Date:** 2026-07-24
**Status:** shipped — PR #368, merged 2026-08-16 · historical draft, see §0
**Issue:** #323 (closed 2026-08-16)
...
shipped. Kept as the design record linked from #323.

- **Shipped in:** PR #368 "Isolate the pytest suite from the deployed .env
  (LFG_SKIP_DOTENV) (#323)" (`4fd434f`), merged 2026-08-16.
- **Followed this design:** closely. Seam B landed as the `LFG_SKIP_DOTENV` gate
  plus the root-`conftest.py` chokepoint carrying the drafted pin list verbatim
  (same throwaway seed `sEdTM1uX8pu2do5XvTnutH6HsouMaM2`, same
  `_require(...)`-mandatory vars, all `setdefault`). Seam A landed as the
  CLAUDE.md "Test env isolation (#323)" subsection with both convention rules.
  Open question 1 resolved to inline conftest pins (no `tests/.env.test`); open
  question 3 resolved to leave the ~116 per-file preambles in place as harmless
  no-ops.
- **Diverged:**
  - The gate is a shared helper, `lfg_core/envload.py::load_dotenv_unless_skipped()`,
    rather than an inline conditional in `config.py` as sketched.
    `lfg_core/db_path.py` keeps its own inlined copy of the same check because
    that module must stay free of `lfg_core` imports.
  - Applied wider than drafted: the three surface configs
    (`surfaces/discord_bot/config.py`, `surfaces/telegram_bot/config.py`,
    `surfaces/x_bot/config.py`) were converted to the gated loader too. The draft
    named only `lfg_core/config.py`.
  - The "run the full suite twice, with and without a hostile `.env`" audit was
    automated instead of run by hand: `tests/test_config_dotenv_gate.py` proves
    the gate in both directions via a subprocess with a planted `.env`, and
    `tests/test_env_isolation.py` is deliberately preamble-free so it would fail
    at collection if conftest alone did not suffice.
  - `tests/test_env_isolation.py::test_frozen_constant_not_env_file_masked`
    asserts *consistency* (`config.BULK_MINT_UI_ENABLED is config.env_flag(...)`)
    rather than the draft's hardcoded `is False`, so the deliberate
    explicit-shell-export escape hatch cannot fail the suite. The shipped-default
    assertion is split into a separate test against the raw
    `BULK_MINT_UI_ENABLED_DEFAULT`.
  - Open question 2 (a lint rule or meta-test forbidding assertions on frozen
    config constants) was not implemented; the written convention is the only
    enforcement.
  - CLAUDE.md's shipped wording adds a rule the draft did not state explicitly:
    never add a bare `load_dotenv()` in code a test can import — call
    `load_dotenv_unless_skipped()` instead.
- **Live behaviour is now documented in:** the "Test env isolation (#323)"
  section of `CLAUDE.md`, the `lfg_core/envload.py` module header, and the
  isolation preamble comment in the root `conftest.py`.
