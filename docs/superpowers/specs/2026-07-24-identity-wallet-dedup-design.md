...
**Date:** 2026-07-24
**Status:** shipped — PR #395, merged 2026-08-18 · historical draft, see §0
**Issue:** #206 (closed 2026-08-18)
...
shipped. Kept as the design record linked from #206.

- **Shipped in:** PR #395 "feat(identity): wallet_links history + same-human
  bucket resolver" (`f715b95`), merged 2026-08-18.
- **Followed this design:** in outline only. A bucket layer did land in
  `lfg_service/identity.py`, computed on demand rather than materialized,
  deriving membership only from proven wallets, matching wallets verbatim, and
  changing no live behaviour (the shipped sponsored free mint gates per-wallet on
  the SourceTag archive and was left untouched). Almost every concrete decision
  below it differs.
- **Diverged:**
  - **Schema.** This draft's central claim was "no schema change —
    `identities.wallet` is the edge set", and its "What this deliberately does NOT
    change" list names "No `wallet_links` table". The shipped PR agreed the table
    did not exist on `main`, then created it: an append-only
    `wallet_links(platform, platform_user_id, wallet, linked_at)` written
    alongside every `identity.link()`, created and seeded from existing
    `identities` rows in `ensure_identities_table()` and by the legacy `Users`
    migration.
  - **Bucket semantics.** The draft's bucket was one wallet, resolved in a single
    `resolve()` hop — so a user who re-linked a different wallet silently left the
    old bucket. Shipped `identity.bucket_for()` runs a connected-components BFS
    over the bipartite identity–wallet graph, making linkage transitive (A–w1–B,
    B–w2–C is one bucket) and permanent (an old, since-replaced wallet still
    links). `bucket_id` is the JSON encoding of the lexicographically smallest
    member key, not a wallet.
  - **API names and shape.** `account_bucket` / `bucket_members` / `same_human`
    were never built (no such symbols in the tree). The shipped surface is
    `bucket_for()` returning `{bucket_id, identities, wallets}`, `same_bucket(a,
    b)`, and `bucket_overlaps(...)` — the last a per-human gating primitive this
    draft did not propose, which answers "has any member of this caller's bucket
    already claimed?" and can only widen a deny set, never grant.
  - **Error posture.** This draft treated `None` as the only negative result.
    Shipped code raises `BucketLookupError` on any DB failure — a deliberate
    exception to the module's never-raises style, so a gate can never read an
    infrastructure error as "no bucket".
  - **Seam B not shipped.** `GET /api/account`
    (`lfg_service/app.py::handle_account`) still returns `{wallet, identities}`;
    there is no `bucket_size` / `platforms` enrichment.
  - **Seam C not shipped.** There is no `scripts/audit_identity_buckets.py`.
  - Open question 1 ("fold into #207?") was resolved by shipping both: #207's
    first-class profiles landed later the same day in PR #396 on top of this
    resolver — `_bucket_on_conn` exists so `ensure_profile_for` can resolve a
    bucket inside its own `BEGIN IMMEDIATE` transaction.
- **Live behaviour is now documented in:** the bucket comment block above
  `bucket_for` in `lfg_service/identity.py` (on-demand storage decision,
  transitivity, leaky-by-design caveat) and the PR #395 description.
