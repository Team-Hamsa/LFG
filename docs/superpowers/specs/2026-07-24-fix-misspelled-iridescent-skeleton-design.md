...
**Date:** 2026-07-24
**Status:** shipped — PR #359, merged 2026-08-16 · historical draft, see §0
**Issue:** #301 (closed 2026-08-16)
...
shipped. Kept as the design record linked from #301.

- **Shipped in:** PR #359 "fix(data): correction tooling for misspelled
  'Iridescent Skeleton' Body value (#301)" (`d914fcf`), merged 2026-08-16.
- **Followed this design:** yes — Option 2 (metadata-only `NFTokenModify`, no
  image recomposition), `scripts/fix_iridescent_body.py` dry-run by default with
  `--apply`, an on-ledger mutability check before any modify, a fresh CDN stem,
  ledger-first-then-mirrors ordering, a `reports/` journal, and the narrow
  layer-tree denylist guard.
- **Diverged:**
  - The pure rewrite this draft sketched inline as step 3 was extracted into its
    own module: `lfg_core/body_fix.py::rewrite_body_value` deep-copies and returns
    `(meta, changed)`, and the `BAD` / `GOOD` constants live there rather than in
    the script.
  - The recurrence guard landed inside the existing
    `scripts/validate_trait_config.py` (`_TYPO_BODY_PREFIX`), so the pre-push / CI
    `validate-trait-config` hook rejects any `layers/<body>/Body/Iridescent *`
    file — not as a sibling check.
  - The script adds guards this draft did not specify: it refuses to run when
    `--network` disagrees with `config.XRPL_NETWORK`, treats an unavailable
    `nft_info` as fail-closed (no modify), and syncs the index row from the
    `nft_info`-confirmed owner rather than the possibly-stale index snapshot.
  - **The PR shipped tooling only — nothing was executed against mainnet.** The
    two-token correction is a separate ops step (`--apply` on the box, then
    `scripts/audit_trait_files.py --network mainnet`). This repository carries no
    record of whether that run has happened, so open question 2 (what to do if
    edition 77's live token is non-mutable) may still be unanswered; the script
    SKIPs and reports such a token, pointing at `scripts/convert_to_mutable.py`.
- **Live behaviour is now documented in:** the module docstring of
  `scripts/fix_iridescent_body.py` (invocation and safety posture) and the ops-step
  block in the PR #359 description.
