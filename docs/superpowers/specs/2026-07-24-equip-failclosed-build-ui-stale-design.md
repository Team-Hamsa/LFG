...
**Date:** 2026-07-24
**Status:** shipped — PR #351, merged 2026-08-06 · historical draft, see §0
**Issue:** #316 (closed 2026-08-06)
...
shipped. Kept as the design record linked from #316.

- **Shipped in:** PR #351 "fix(economy): honest Build UI reconcile for fail-closed
  equip outcomes" (`1eb382e`), merged 2026-08-06.
- **Followed this design:** closely — both seams landed as written.
  `EquipSession.resolution` (`lfg_core/economy_flow.py`) classifies every terminal
  branch `committed` / `reverted` / `uncertain`, with the generic outer-catch left
  `None`; `webapp/economy_api.py::economy_session_dict` exposes it `getattr`-safe
  on the `equip` branch only; `webapp/client/build_pure.js::saveOutcome` is the
  pure three-way classifier; `app.js::saveBuild` shows a distinct message, adds
  the character to `reconcileUncertainIds`, banners it in the dressup header, and
  gates `stagePendingEquip` / `saveBuild` on it.
- **Diverged:**
  - `saveOutcome` takes the whole terminal session object, not the resolution
    string this draft sketched (`buildPure.saveOutcome(resolution)`), and also
    treats `state === 'done'` as `committed` — so a success reported without a
    `resolution` still classifies correctly.
  - `resolution` is additionally persisted into the journal record
    (`EquipSession._record`), which this draft did not call for; a recovered
    `equip_sync_indeterminate` / `failed_revert` record would otherwise read as a
    clean revert.
  - The client pins the submitted `nft_id` across the save poll
    (`submittedNftId` in `saveBuild`), so the flag and banner attach to the
    character that was actually saved rather than whatever is selected when the
    poll returns. Not in this draft.
  - Open question 1 (grep `ECONOMY_RECORDS_DIR` first, and possibly ship the
    server signal alone) was not taken up — the full client banner and gate
    shipped.
  - Open question 3 resolved narrowly: the flag clears only on a wardrobe refresh
    (`openDressup`'s `/api/economy` fetch), not on any successful refetch —
    `saveBuild`'s own `finally` refetch deliberately leaves it set.
  - Cache busters moved with the change (`app.js?v=34→36` across the PR's two
    commits, `build_pure.js` import `v=26→27`); later, unrelated work has since
    bumped `app.js` further (`v=59` at time of this review).
  - Issue option 2 (durable server-side gate) and option 3
    (chain-reconcile-on-read) stayed out of scope, as proposed here.
- **Live behaviour is now documented in:** the `resolution` comment block on
  `EquipSession` and the status table in the `lfg_core/economy_flow.py` module
  docstring; `webapp/client/build_pure.js::saveOutcome`.
