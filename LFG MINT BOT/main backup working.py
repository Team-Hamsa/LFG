...
async def create_payment_request(destination: str) -> dict:
    """Create a XUMM payment request for 1 token"""
    ...
    # Create the transaction JSON for token payment
    transaction_json = {
    ...
    logging.info(f"Payment transaction JSON prepared: {json.dumps(transaction_json, indent=2)}")
    ...
    try:
        logging.info(f"Sending request to XUMM API: {XUMM_API_URL}")
        response = await asyncio.to_thread(
            requests.post,
            XUMM_API_URL,
            json=payload,
        ...
        logging.info(f"XUMM API response status: {response.status_code}")
        
        response_data = response.json()
        logging.info(f"XUMM API response data: {json.dumps(response_data, indent=2)}")
...
async def check_payment_status(uuid: str) -> bool:
    """Check if a XUMM payment has been completed"""
    try:
        logging.info(f"Checking payment status for UUID: {uuid}")
        response = await asyncio.to_thread(sdk.payload.get, uuid)
        logging.info(f"Payment status response: {response.to_dict()}")
        return response.meta.signed and response.meta.resolved
    except Exception as e:
...
class MintView(View):
    ...
    async def mint_button(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            embed.set_image(url=payment_data['qr_url'])
            ...
            # If we get here, payment timed out
    ...
    async def trustline_button(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            # Wait for trustline setup (check every 5 seconds for up to 5 minutes)
            for _ in range(60):  # 5 minutes = 60 * 5 seconds
