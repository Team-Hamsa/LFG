...
- **Closet Market** — sell a trait straight out of your Closet with **no signature at all**: an ask encumbers the asset in place instead of moving it, and buyers bid by locking BRIX in an XRPL `TokenEscrow` under a PREIMAGE-SHA-256 condition that only the fill can release ([#443](../../issues/443)). Live in production since **2026-09-18** (`CLOSET_MARKET_ENABLED=1`), replacing the Extract → list → accept round trip for Closet assets
...
- [x] [#428 — Marketplace: royalty-refund campaign for sales settled through LFG](../../issues/428) (closed 2026-09-21)
