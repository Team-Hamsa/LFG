...
def _clean_env() -> dict[str, str]:
    """os.environ minus GIT_* — when pytest itself runs inside a git hook
    (the pre-push gate), git exports GIT_DIR etc., which would otherwise make
    the temp repos' git commands (and the shim's rev-parse) target the OUTER
    repo instead of the temp one."""
