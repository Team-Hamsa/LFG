...
def test_start_activates_campaign_and_reverifies(dbs, fake_reverify, capsys):
    app_db, hist_db = dbs
    assert run("start", "--network", "testnet", "--actor", "josh") == 0
...
def test_start_reports_failed_reverify_but_leaves_campaign_active(dbs, monkeypatch, capsys):
    ...
    monkeypatch.setattr(cli, "_clio_client", lambda: _NullClient())
    assert run("start", "--network", "testnet", "--actor", "josh") == 3
...
def test_start_skip_reverify(dbs, monkeypatch, capsys):
    ...
    assert run("start", "--network", "testnet", "--actor", "josh", "--skip-reverify") == 0
    assert "reverify: skipped" in capsys.readouterr().out


def test_stop_and_status(dbs, fake_reverify, capsys):
    app_db, hist_db = dbs
    assert run("start", "--network", "testnet", "--actor", "josh") == 0
    assert run("stop", "--network", "testnet", "--actor", "josh") == 0
    capsys.readouterr()
...
def test_refuses_network_mismatch(dbs, fake_reverify, capsys):
    assert run("start", "--network", "mainnet", "--actor", "josh") == 2
...
def test_start_requires_actor(dbs, fake_reverify):
    with pytest.raises(SystemExit):
        run("start", "--network", "testnet")
...
def test_start_surfaces_library_refusal(dbs, fake_reverify, monkeypatch, capsys):
    ...
    assert run("start", "--network", "testnet", "--actor", "josh") == 1
