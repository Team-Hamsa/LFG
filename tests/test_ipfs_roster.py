...
def test_img_proxy_accepts_ipfs_gateway_host(monkeypatch):
    ...
    # Bypass the local-archive fast path (#153): on a box with the real
    # images_<net>/ archive this edition would be served from disk and
    # _fetch_cdn never reached — the test is about the proxy fallback.
    monkeypatch.setattr(server.image_archive, "edition_for_url", lambda *_: None)
...
def test_img_proxy_resolves_raw_ipfs_uri(monkeypatch):
    ...
    monkeypatch.setattr(server, "_fetch_cdn", fake_fetch)
    # Bypass the local-archive fast path — see test_img_proxy_accepts_ipfs_gateway_host.
    monkeypatch.setattr(server.image_archive, "edition_for_url", lambda *_: None)
