...
def _x_share_text(kind: str, nft_number: Any) -> str:
    ...
    if kind == "equip":
        return (
            f"I just restyled LFG #{n}! \U0001f9f1 @letseffinggo #XRPL"
            if n is not None
            else "I just restyled my LFG! \U0001f9f1 @letseffinggo #XRPL"
        )
    if kind == "assemble":
        return (
...
async def handle_x_share(request):
    ...
    if kind not in ("mint", "swap", "assemble", "equip"):
...
async def handle_closet(request):
    ...
    try:
        ...
        # Definitive failure of a single step (mint/offer returned falsey) —
        # usually upstream rippled flakiness (#385/#386). A retry is safe:
        # ensure_closet records nothing when the mint never committed, and
        # records the minted token BEFORE the offer step when it did, so a
        # re-run either mints fresh or re-offers the SAME token — never a
        # duplicate mint. Surface a structured, retryable 503, not a dead 502.
...
async def handle_mint_start(request):
    ...
    if reservation is not None and reservation.reason == "eligibility_pending":
        ...
        if session.state != mint_flow.AWAITING_PAYMENT:
            # A cancel landed while the reservation ran. That terminal state is
            # the session's real outcome — reporting "check back" over it would
            # show retry-later messaging for a session the user already killed.
            mint_flow.settle_headroom(session)
            return web.json_response(session.to_dict())
        session.state = mint_flow.FAILED
...
handle_assemble_start = _economy_post(
    ...
    lambda uid, w, b, tok: economy_api.start_assemble(
        uid, w, b["nft_id"], b["body"], b["chosen"], user_token=tok
    ),
    lambda w, b: mock_economy.INSTANCE.assemble(w, b["nft_id"], b["body"], b["chosen"]),
)
