...
async def _offer_or_skip(
    nft_id: str, owner: str, platform: str = memos.PLATFORM_BACKEND
) -> str | None:
    if owner == config.SWAP_ISSUER_ADDRESS:
        return _SELF_OFFER_SKIPPED
    return await xrpl_ops.create_nft_offer(nft_id, owner, amount="0", platform=platform)
...
async def _accept_or_skip(
    ...
    return await xumm_ops.create_accept_offer_payload(
        offer_id, user_token=user_token, account=owner, platform=platform
    )
...
async def _closet_modify(
    nft_id: str, owner: str, url: str, platform: str = memos.PLATFORM_BACKEND
) -> closet_token.ModifyReceipt | None:
    ...
    try:
        result = await xrpl_ops.modify_nft(
            nft_id, owner, url, return_details=True, platform=platform
        )
...
def build_economy_deps(
    ...
    ``platform`` (default: the session's, via memos.backend_platform) labels every op."""
    ...
    return economy_flow.EconomyDeps(
        ...
        char_modify_fn=lambda nft_id, owner, url: xrpl_ops.modify_nft(
            nft_id, owner, url, platform=platform
        ),
        char_burn_fn=lambda nft_id, owner: xrpl_ops.burn_nft(
            nft_id, owner or None, platform=platform
        ),
        ...
        trait_burn_fn=lambda nft_id, owner: xrpl_ops.burn_nft(
            nft_id, owner or None, platform=platform
        ),
        ...
        # #522: refuse a full Closet overwrite while the mirror is behind the
        # chain, judged against the listener's history archive (read-only).
        history_db_path=history_store.history_db_path(config.ECONOMY_NETWORK),
    )
