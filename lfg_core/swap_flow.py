...
async def _build_and_upload(
    ...
    image_url, video_url = await swap_compose.upload_output(
        ...
        keep_still=image_archive.pending_still_path(config.XRPL_NETWORK, num),
    )
    return image_url, video_url, new_burn


def _archive_stills(items: list[dict[str, Any]]) -> None:
    ...
    for item in items:
        image_archive.promote_still(config.XRPL_NETWORK, item["nft"]["number"])


def _discard_stills(items: list[dict[str, Any]]) -> None:
    """Drop staged stills for items whose swap did not finalize."""
    for item in items:
        image_archive.discard_still(config.XRPL_NETWORK, item["nft"]["number"])
    ...
    try:
        ...
        if burn_items:
            ...
            for i, item in enumerate(burn_items):
                ...
                _archive_stills([other])
                _discard_stills([item])
        ...
        for nft in (session.nft1, session.nft2):
            image_archive.discard_still(config.XRPL_NETWORK, nft["number"])
        session.state = FAILED
        session.error = str(e)
